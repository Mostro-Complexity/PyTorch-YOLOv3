...
class VOC2007(Base):
    ...
    def evaluate(self, path_to_results_dir: str, image_ids: List[str], bboxes: List[List[float]], classes: List[int], probs: List[float]) -> Tuple[float, str]:
        ...
        for c in range(1, VOC2007.num_classes()):
            ...
            try:
                ...
                # NOTE: for windows path bugs
                # use os.path.join(path_to_annotations_dir, '{}.xml')
                # instead of os.path.join(path_to_annotations_dir, '{:s}.xml')
                _, _, ap = voc_eval(detpath=os.path.join(path_to_results_dir, 'comp3_det_test_{}.txt'.format(category)),
    ...
    @staticmethod
    def num_classes() -> int:
        return len(VOC2007.LABEL_TO_CATEGORY_DICT)
