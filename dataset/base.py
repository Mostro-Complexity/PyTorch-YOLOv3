...
class Base(torch.utils.data.dataset.Dataset):
    ...
    OPTIONS = [
        'voc2007', 'coco2017', 'voc2007-cat-dog', 'coco2017-person',
        'coco2017-car', 'coco2017-animal', 'tiny-person'
    ]
    ...
    def from_name(name: str) -> Type['Base']:
        ...
        elif name == 'tiny-person':
            from dataset.tiny_person import TinyPerson
            return TinyPerson
        else:
            raise ValueError
    ...
    # @staticmethod
    # def preprocess_with_size(image: PIL.Image.Image, size: Tuple[int, int]):
    #     """
    ...
    #     scale = (size[0] / original_size[0], size[1] / original_size[1]) * 2

    #     transform = transforms.Compose([
    #         transforms.Resize((size[0], size[1])),  # interpolation `BILINEAR` is applied by default
    #         transforms.ToTensor(),
    #         transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    #     ])
    #     image = transform(image)
    ...
    @staticmethod
    def padding_collate_fn(batch: List[Tuple[int, Tensor, Tensor, Tensor]]) -> Tuple[List[str], Tensor, Tensor, Tensor, Tensor]:
        image_id_batch, image_batch, bboxes_batch, labels_batch = zip(*batch)
        ...
        return image_id_batch, padded_image_batch, padded_bboxes_batch, padded_labels_batch
